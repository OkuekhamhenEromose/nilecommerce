...
class RegistrationSerializer(serializers.ModelSerializer):
    ...
    def validate(self,data):
        if data['password1'] != data['password2']:
            raise serializers.ValidationError('password does not match')#for human readable errors
        if not data['fullname']:
            raise serializers.ValidationError('fullname does not match')#for human readable errors
        return data
